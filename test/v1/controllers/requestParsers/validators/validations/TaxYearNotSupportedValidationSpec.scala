...
import v1.models.errors.{InvalidTaxYearParameterError, RuleTaxYearNotSupportedError}
  ...
  "validate" should {
    "return no errors" when {
      ...
      "a tax year that is 2023-24 is supplied" in {
        ...
        val validationResult = TaxYearNotSupportedValidation.validateTys(validTaxYear)
        validationResult.isEmpty shouldBe true
      }
    ...
    "return the given error" when {
      ...
      "a tax year below 2022-23 is supplied" in {
        ...
        val validationResult = TaxYearNotSupportedValidation.validateTys(invalidTaxYear)
        validationResult.isEmpty shouldBe false
        validationResult.length shouldBe 1
