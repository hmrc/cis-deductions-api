...
object DownstreamUri {
  ...
  def DesUri[Resp](value: String)(using appConfig: SharedAppConfig): DownstreamUri[Resp] =
    withStandardStrategy(value, appConfig.desDownstreamConfig)

  def IfsUri[Resp](value: String)(using appConfig: SharedAppConfig): DownstreamUri[Resp] =
    withStandardStrategy(value, appConfig.ifsDownstreamConfig)

  def HipUri[Resp](path: String)(using appConfig: SharedAppConfig): DownstreamUri[Resp] =
    DownstreamUri(path, DownstreamStrategy.basicAuthStrategy(appConfig.hipDownstreamConfig))

  def DesToHipMigrationUri[Resp](path: String, switchName: String)(using appConfig: SharedAppConfig): DownstreamUri[Resp] = {
