...
class DeleteControllerISpec extends IntegrationBaseSpec {
  ...
  "Calling the delete endpoint" should {
    ...
    "return error according to spec" when {
      case class RequestData(nino: String, submissionId: String, taxYear: String)
      ...
      "validation error" when {
        ...
        def validationErrorTest(requestData: RequestData, expectedStatus: Int, expectedBody: MtdError): Unit = {
          s"validation fails with ${expectedBody.code} error" in new NonTysTest {
            ...
            override val taxYear: String      = requestData.taxYear
  ...
  private trait Test {
    ...
    def mtdRequest(): WSRequest = {
      setupStubs()
      buildRequest(s"/$nino/amendments/$submissionId/$taxYear")
        .withHttpHeaders(
